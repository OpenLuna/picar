...
def streaming():
    ...
    while True: #connect to server loop
        try:
            conn = httplib.HTTPConnection('pelji.se', 80, timeout=1) #int(config["stream port"]), timeout = 1)
            #conn.set_debuglevel(1)
            conn.putrequest("POST", "/pub?id=test") #?" + urllib.urlencode({"token": config["secret key"], "name": config["name"]}))
